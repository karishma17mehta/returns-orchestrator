...
(`app/agents/`): a **fraud analyst**, a **policy compliance specialist**
(grounded in your indexed brand policy PDFs via `POLICY_CHUNKS_PKL`), and a
...
.venv/bin/pytest                          # 67 tests, no network needed
.venv/bin/uvicorn app.main:app --reload   # docs at http://localhost:8000/docs
```

Environment: `RETURNS_DB` (SQLite path, `:memory:` for ephemeral),
`RETURNS_API_KEYS`, `CARRIER_WEBHOOK_SECRET`, `POLICY_CATALOG_CSV`,
`POLICY_CHUNKS_PKL`, `OPENAI_API_KEY`, `LOG_LEVEL`.
